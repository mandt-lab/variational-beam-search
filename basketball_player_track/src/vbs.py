...
class BeamSearchHelper:
    ...
    def __init__(self, save_folder, beam_size, diffusion, jump_bias,
                 ...
                 max_decision_length=2000, temper_every_iter=False, verbose=False):
    ...
    def print_log(self, *args, **kwargs):
        if self.verbose and (self.task_id % self.permanent_store_every_tasks == 0):
            print("[{}]".format(datetime.now()), *args, **kwargs)
            sys.stdout.flush()
            ...
            for h, parent_hypo_res in enumerate(self.latest_task_res):
                ...
                for s_t in s_set:
                    hypo_args = [
                        ...
                        s_t, 
                        self.diffusion, 
                        parent_hypo_res['param'],
                        *data_wrapper]
                    hypos_args.append(hypo_args)
        self._cur_hypos_args = hypos_args
        return hypos_args

    def absorb_new_evidence_and_prune_beams(self, elbos, param_set):
        log_post_prob = self._calculate_posterior_probabilities(elbos)
        self._prune_beams(param_set, log_post_prob)
    ...
    def _prune_beams(self, param_set, log_post_prob):
        ...
        # update current task
        self.current_task_res = []
        for i, (param, log_prob, hypo) in enumerate(
                zip(param_set, log_post_prob, self._cur_hypos_args)):
            if not self.latest_task_res:
