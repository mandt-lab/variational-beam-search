...
if __name__ == '__main__':
    for jump_prior in [-10]: # [-5120, -1280, -320, -40] [-2560, -960, -640] [-480, -240, -160, -120, 0]
        ...
        DATA_FOLDER = "./data-n-ckpt/crecords/dat"
        IN_DATAPATH_TEMPLATE = DATA_FOLDER + '/npos_%03d.bin.gz'
        VAL_DATAPATH_TEMPLATE = DATA_FOLDER + '/val_npos_%03d.bin.gz'
        CHECKPOINT_FOLDER = ( # saved folder
            './v%d_d%d_b%d_p%d_diff%f_crecords/' % (MAX_VOCAB_SIZE,   
                                                 DIM, 
                                                 BEAM_SIZE, 
                                                 JUMP_BIAS, 
                                                 DIFFUSION))
        OUT_DIR_TEMPLATE = CHECKPOINT_FOLDER + 'crecords%03d_b%s_s%s/' # year, beam_id, s_t
        PREV_PARAM_PATH_TEMPLATE = CHECKPOINT_FOLDER + '%s_%03d.npy' # beam_mu/beam_std, year
        BEAM_VAR_FOLDER = CHECKPOINT_FOLDER + '/beam_results/'
        CONTEXT_VECTOR_CKPT_PATH = (
            './data-n-ckpt/crecords' 
            '/crecords_entire_timesteps_v%d_d%d/checkpoint-10000' % (MAX_VOCAB_SIZE, DIM))
