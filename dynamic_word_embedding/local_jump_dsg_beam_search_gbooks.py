    ...
    for jump_prior in [-10]: 
        ...
        def dsg_beam_search(year_list, s_dim, beam_size=1, jump_bias=-100):
            ...
            for (i, year) in enumerate(year_list):
                if year == start_year:
                    # FOR continue after unintentional break
                    # log_num_beams += (np.log(2) * 68)
                    # beam_mu = np.load(PREV_PARAM_PATH_TEMPLATE % ('beam_mu', year))
                    # beam_std = np.load(PREV_PARAM_PATH_TEMPLATE % ('beam_std', year))
                    # beam_log_s = np.load(BEAM_VAR_FOLDER + 'beam_log_s_v%d_d%d_b%d_y%d.npy' % (MAX_VOCAB_SIZE, DIM, beam_size, year))
                    # beam_decision = np.load(BEAM_VAR_FOLDER + 'beam_decision_v%d_d%d_b%d_y%d.npy' % (MAX_VOCAB_SIZE, DIM, beam_size, year))
                    # continue 
                    with Pool(1) as p:
